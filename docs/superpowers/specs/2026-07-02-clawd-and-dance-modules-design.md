...
- `/dance` の `ms` 正規化（`clampDanceMs`）: `ms<=0`（未指定・不正）は既定 6000ms、それ以外は `[500, 30000]` にクランプ（下限未満は 500、上限超過は 30000）
- `/clip` の `ms`: `ms<=0`（未指定・不正）は既定 4000ms、それ以外は上限 60000ms にクランプ（`g_forcedUntil` の破壊防止）
